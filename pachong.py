...
#整理好的数据存入txt
with open(path.abspath(path.join(path.dirname(__file__), 'ul.txt')), 'w') as f:
	f.write(str(li))
...
x = [subli[0] for subli in li]  # X轴
xtext = [subli[1] for subli in li]  # X轴刻度标签
y = [subli[3] for subli in li]  # Y轴 阅读量
savepath1= path.abspath(path.join(path.dirname(__file__), 'line1.png'))
...
savepath2= path.abspath(path.join(path.dirname(__file__), 'line2按章.png'))
